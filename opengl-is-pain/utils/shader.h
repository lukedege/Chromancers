...
public:
        ...
        program{ glCreateProgram() }, glMajorVersion{glMajor}, glMinorVersion{glMinor}
    {
        ...
        // the size and content of the string is the same so IDK WHY TF IT ISNT LINKING lighting.frag
        //GLsizei bufSize = 16384, realSize = 0;
        //GLchar* pFrag = new GLchar[16384];
        //glGetShaderSource(fragmentShader, bufSize, &realSize, pFrag);

        try {
            glLinkProgram(program);
        }
        catch (std::out_of_range e) { auto x = glGetError(); checkLinkingErrors(); std::cout << e.what(); }
        auto x = glGetError(); checkLinkingErrors();
