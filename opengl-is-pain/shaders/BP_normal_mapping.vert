...
void calcPointLI(uint plIndex)
{
	...
	vs_out.vwPointLightPos[plIndex] = vec3(viewMatrix * vec4(pointLights[plIndex].position, 1)); // convert light position from world to view coordinates
}
...
void main()
{
	...
	vs_out.vwFragPos = vec3(viewMatrix * modelMatrix * vec4(position, 1));
